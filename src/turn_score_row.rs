...
impl FactoryComponent for TurnScoreRow {
    ...
    fn update(&mut self, message: TurnScoreRowInput, sender: FactorySender<Self>) {
        match message {
            ...
            TurnScoreRowInput::ScoreChanged(player_index, score) => {
                sender.output(TurnScoreRowOutput::ScoreChanged(self.turn_row_index.clone(), player_index, score));
            }
        }
    }
    ...
    fn forward_to_parent(output: Self::Output) -> Option<Self::ParentInput> {
        Some(match output {
            TurnScoreRowOutput::ScoreChanged(turn_row_index, player_index, score) => ScorePageInput::ScoreChanged(turn_row_index, player_index, score),
        })
    }
